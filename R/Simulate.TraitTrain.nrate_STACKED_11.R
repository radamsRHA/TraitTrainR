#' Simulate.TraitTrain.nrate_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) nrate model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the nrate model
...
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of nrate)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of nrate)
...
#' 
#' ##########################################################################
#' # simulate under nrate model with STACKED 11 (nrate + lrates + AncShift) #
#' #########################################################################
#' Simulate.TraitTrain.nrate_STACKED_11(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
########################################
# Simulate.TraitTrain.nrate_STACKED_11 #
#######################################
Simulate.TraitTrain.nrate_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.AncestralState, vector.nrate_time, vector.nrate_rate, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  ##############################
  handle.Phylogeny_TRANSFORM_nrate_lrates <- rescale(handle.Phylogeny_TRANSFORM_nrate, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_nrate_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
