#' Simulate.TraitTrain.delta_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) delta model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the delta model
#' @param numeric.delta Numeric value of the delta parameter of the delta model
#' @param vector.STACK_lrate_nodes Vector that includes all nodes for a STACKED lrates model (on top of delta)
#' @param vector.STACK_lrate_rates Vector that includes all rate values for each node in vector.STACK_lrate_nodes for a STACKED lrates model (on top of delta)
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of delta)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of delta)
...
#' 
#' ############################################################################
#' # simulate under delta model with STACKED 11 (delta + lrates + AncShift) #
#' ############################################################################
#' Simulate.TraitTrain.delta_STACKED_11(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#########################################
# Simulate.TraitTrain.delta_STACKED_11 #
#########################################
Simulate.TraitTrain.delta_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.delta, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  ##############################
  handle.Phylogeny_TRANSFORM_delta_lrates <- rescale(handle.Phylogeny_TRANSFORM_delta, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_delta_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
