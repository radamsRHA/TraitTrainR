...
Simulate.TraitTrain.delta_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.delta, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # transform via delta #
  ########################
  handle.Phylogeny_TRANSFORM_delta <- phytools::rescale(handle.Phylogeny, "delta")(sigsq = numeric.Sig2, delta = numeric.delta)
  
  ##############################
  # transform via STACK lrates #
  ##############################
  handle.Phylogeny_TRANSFORM_delta_lrates <- phytools::rescale(handle.Phylogeny_TRANSFORM_delta, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_delta_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
