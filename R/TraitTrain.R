...
#  if(exists('rescale', where='package:phytools', mode='function')){rescale <- phytools::rescale; print("Found rescale function in the installed package phytools")}
  handle.Phylogeny_DEPTH <- rescale(handle.Phylogeny, model = "depth")(depth = 1)
    ...
    if (string.PrimaryModel == "BM"){
      ...
      if (CHECK.STACK_00 == T){
        for (j in 1:numeric.NumberModelReplicates){
          ...
          # CONVERT TO PIC #
          ##################
          if (logical.PIC == T){matrix.PIC_DEPTH <- matrix.PIC <- matrix(nrow = (numeric.NumberOfSpecies-1), ncol = numeric.NumberOfTraits); for (p in 1:numeric.NumberOfTraits){matrix.PIC[,p] <- pic(x = matrix.TraitData_RAW[,p], phy = handle.Phylogeny); matrix.PIC_DEPTH[,p] <- pic(x = matrix.TraitData_RAW[,p], phy = handle.Phylogeny_DEPTH)}; matrix.RESULTS_PIC[numeric.COUNTER,] <- c(matrix.PIC, i); matrix.RESULTS_PIC_DEPTH[numeric.COUNTER,] <- c(matrix.PIC_DEPTH, i)}}
      }
  ...
  if (logical.PROJECT == T){
    ...
    # Decompose vcv #
    #################
    matrix.C <- vcv(phy = handle.Phylogeny); matrix.U <- eigen(x = matrix.C)$vectors; matrix.W <- matrix(0, nrow = numeric.NumberOfSpecies, ncol = numeric.NumberOfSpecies); diag(matrix.W) <- eigen(x = matrix.C)$values
    matrix.P <- solve(matrix.U %*% (matrix.W %XXX% (0.5)) %*% t(matrix.U)); colnames(matrix.P) <- rownames(matrix.P) <- handle.Phylogeny$tip.label
    
    matrix.C <- vcv(phy = handle.Phylogeny_DEPTH); matrix.U <- eigen(x = matrix.C)$vectors; matrix.W <- matrix(0, nrow = numeric.NumberOfSpecies, ncol = numeric.NumberOfSpecies); diag(matrix.W) <- eigen(x = matrix.C)$values
