#' Simulate.TraitTrain.OU_STACKED_01: Function to simulate trait data under the STACK (shifted) OU model
#'
...
#' 
#' ###########################################################
#' # simulate under BM model with STACKED 01 (BM + AncShift) #
#' ###########################################################
#' Simulate.TraitTrain.OU_STACKED_01(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#####################################
# Simulate.TraitTrain.OU_STACKED_01 #
#####################################
Simulate.TraitTrain.OU_STACKED_01 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.Alpha, numeric.AncestralState, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_OU, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
