#' Simulate.TraitTrain.trend_STACKED_01: Function to simulate trait data under the STACK (+AncShift) trend model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the trend model
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of trend)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of trend)
...
#' ##########################################################################
#' Simulate.TraitTrain.trend_STACKED_01(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
########################################
# Simulate.TraitTrain.trend_STACKED_01 #
########################################
Simulate.TraitTrain.trend_STACKED_01 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.AncestralState, numeric.slope, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_trend, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
