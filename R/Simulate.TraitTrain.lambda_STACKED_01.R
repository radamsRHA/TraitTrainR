#' Simulate.TraitTrain.lambda_STACKED_01: Function to simulate trait data under the STACK (+ AncShift) lambda model
#'
...
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STACKED AncShift model (on top of lambda)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STACKED AncShift model (on top of lambda)
...
#' 
#' ###################################################################
#' # simulate under lambda model with STACKED 01 (lambda + AncShift) #
#' ###################################################################
#' Simulate.TraitTrain.lambda_STACKED_01(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#########################################
# Simulate.TraitTrain.lambda_STACKED_01 #
#########################################
Simulate.TraitTrain.lambda_STACKED_01 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.lambda, numeric.AncestralState, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_lambda, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
