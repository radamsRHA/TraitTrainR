#' Simulate.TraitTrain.BM_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) BM model
#'
...
#' 
#' ####################################################################
#' # simulate under BM model with STACKED 11 (BM + lrates + AncShift) #
#' ####################################################################
#' Simulate.TraitTrain.BM_STACKED_11(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#####################################
# Simulate.TraitTrain.BM_STACKED_11 #
#####################################
Simulate.TraitTrain.BM_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  ##############################
  handle.Phylogeny_TRANSFORM_BM_lrates <- rescale(handle.Phylogeny_TRANSFORM_BM, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_BM_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
