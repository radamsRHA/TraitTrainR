...
Simulate.TraitTrain.depth_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.depth, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  ##############################
  handle.Phylogeny_TRANSFORM_depth_lrates <- phytools::rescale(handle.Phylogeny_TRANSFORM_depth, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_depth_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
