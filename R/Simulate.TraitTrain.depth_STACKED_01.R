#' Simulate.TraitTrain.depth_STACKED_01: Function to simulate trait data under the STACK (+AncShifted) depth model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the depth model
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of depth)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of depth)
...
#' 
#' #################################################################
#' # simulate under depth model with STACKED 01 (Depth + AncShift) #
#' #################################################################
#' Simulate.TraitTrain.depth_STACKED_01(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#######################################
# Simulate.TraitTrain.depth_STACKED_01 #
########################################
Simulate.TraitTrain.depth_STACKED_01 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.depth, numeric.AncestralState, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_depth, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
