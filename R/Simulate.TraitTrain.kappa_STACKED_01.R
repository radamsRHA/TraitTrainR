#' Simulate.TraitTrain.kappa_STACKED_01: Function to simulate trait data under the STACK (+ AncShift) kappa model
#'
...
#' 
#' ##################################################################
#' # simulate under kappa model with STACKED 01 (kappa + AncShift) #
#' ##################################################################
#' Simulate.TraitTrain.kappa_STACKED_01(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
########################################
# Simulate.TraitTrain.kappa_STACKED_01 #
########################################
Simulate.TraitTrain.kappa_STACKED_01 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.kappa, numeric.AncestralState, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_kappa, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
