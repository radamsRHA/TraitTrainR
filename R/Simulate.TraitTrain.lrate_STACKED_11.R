#' Simulate.TraitTrain.lrate_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) lrate model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the lrate model
...
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of lrate)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of lrate)
...
#' 
#' ##########################################################################
#' # simulate under lrate model with STACKED 11 (lrate + lrates + AncShift) #
#' #########################################################################
#' Simulate.TraitTrain.lrate_STACKED_11(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
########################################
# Simulate.TraitTrain.lrate_STACKED_11 #
########################################
Simulate.TraitTrain.lrate_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.AncestralState, vector.lrate_node, vector.lrate_rate, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  ##############################
  handle.Phylogeny_TRANSFORM_lrate_lrates <- rescale(handle.Phylogeny_TRANSFORM_lrate, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_lrate_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
