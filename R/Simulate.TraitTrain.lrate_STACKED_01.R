#' Simulate.TraitTrain.lrate_STACKED_11: Function to simulate trait data under the STACK (+AncShifted)) lrate model
#'
...
#' 
#' #################################################################
#' # simulate under lrate model with STACKED 01 (lrate + AncShift) #
#' #################################################################
#' Simulate.TraitTrain.lrate_STACKED_01(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
########################################
# Simulate.TraitTrain.lrate_STACKED_01 #
########################################
Simulate.TraitTrain.lrate_STACKED_01 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.AncestralState, vector.lrate_node, vector.lrate_rate, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_lrate, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
