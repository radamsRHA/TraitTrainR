#' Simulate.TraitTrain.OU_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) OU model
#'
...
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STACKED AncShift model (on top of OU)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STACKED AncShift model (on top of OU)
...
#' vector.Alpha <- runif(n = numeric.NumberTrainingReps, min = exp(-500), max = exp(1))# vector of parameters for the alpha parameter of the OU Model
#' 
#' #############################
...
#' 
#' ####################################################################
#' # simulate under OU model with STACKED 11 (OU + lrates + AncShift) #
#' ####################################################################
#' Simulate.TraitTrain.OU_STACKED_11(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#####################################
# Simulate.TraitTrain.OU_STACKED_11 #
#####################################
Simulate.TraitTrain.OU_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.Alpha, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  
  ####################
  # transform via OU #
  ####################
  handle.Phylogeny_TRANSFORM_OU <- phytools::rescale(handle.Phylogeny, "OU")(sigsq = numeric.Sig2, alpha = numeric.Alpha)
  
  ##############################
  # transform via STACK lrates #
  ##############################
  handle.Phylogeny_TRANSFORM_OU_lrates <- phytools::rescale(handle.Phylogeny_TRANSFORM_OU, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_OU_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
