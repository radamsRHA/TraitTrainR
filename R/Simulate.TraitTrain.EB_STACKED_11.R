#' Simulate.TraitTrain.EB_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) EB model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the EB model
...
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of EB)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of EB)
...
#' 
#' ####################################################################
#' # simulate under EB model with STACKED 11 (EB + lrates + AncShift) #
#' ####################################################################
#' Simulate.TraitTrain.EB_STACKED_11(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#####################################
# Simulate.TraitTrain.EB_STACKED_11 #
#####################################
Simulate.TraitTrain.EB_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.A, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  ##############################
  handle.Phylogeny_TRANSFORM_EB_lrates <- rescale(handle.Phylogeny_TRANSFORM_EB, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_EB_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
