#' Simulate.TraitTrain.trend_STACKED_10: Function to simulate trait data under the STACK (+lrates) trend model
#'
...
#' ##########################################################################
#' Simulate.TraitTrain.trend_STACKED_10(handle.Phylogeny = handle.PrimatePhylogeny, 
...
########################################
# Simulate.TraitTrain.trend_STACKED_10 #
########################################
Simulate.TraitTrain.trend_STACKED_10 <- function(handle.Phylogeny, numeric.Sig2, numeric.AncestralState, numeric.slope, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, matrix.R){
  
  #######################
  # transform via trend #
  #######################
  handle.Phylogeny_TRANSFORM_trend <- phytools::rescale(handle.Phylogeny, "trend")(sigsq = numeric.Sig2, slope = numeric.slope)
  
  ##############################
  # transform via STACK lrates #
  ##############################
  handle.Phylogeny_TRANSFORM_trend_lrate <- phytools::rescale(handle.Phylogeny_TRANSFORM_trend, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_trend_lrate, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
