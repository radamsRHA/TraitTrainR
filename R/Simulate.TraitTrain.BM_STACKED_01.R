#' Simulate.TraitTrain.BM_STACKED_01: Function to simulate trait data under the STACK (AncShifted) BM model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the BM model
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of BM)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of BM)
...
#' ##########################
#' numeric.NumberTrainingReps <- 1 # same number of replicates for all models in list.SimulationModelSettings
...
#' 
#' ########################################################
#' # simulate under BM model with STACKED (BM + AncSHift) #
#' ########################################################
#' Simulate.TraitTrain.BM_STACKED_01(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
#'                                   handle.Phylogeny = handle.PrimatePhylogeny, numeric.Sig2 = list.SimulationModelSettings[[1]]$vector.Sig2[1], 
#'                                   numeric.AncestralState = list.SimulationModelSettings[[1]]$vector.AncestralState[1], 
#'                                   vector.STACK_AncShiftNode = list.SimulationModelSettings[[1]]$matrix.STACK_AncShiftNode,  
#'                                   vector.STACK_AncShiftValue = list.SimulationModelSettings[[1]]$matrix.STACK_AncShiftValue, 
#'                                   matrix.R = list.SimulationModelSettings[[1]]$list.Rmatrix[[1]])

#####################################
# Simulate.TraitTrain.BM_STACKED_01 #
#####################################
Simulate.TraitTrain.BM_STACKED_01 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.AncestralState, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  
  ####################
  # transform via BM #
  ####################
  handle.Phylogeny_TRANSFORM_BM <- rescale(handle.Phylogeny, "BM")(sigsq = numeric.Sig2)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_BM, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
