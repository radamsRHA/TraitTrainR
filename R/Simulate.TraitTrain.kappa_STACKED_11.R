...
Simulate.TraitTrain.kappa_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.kappa, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  ...
  # transform via kappa #
  ########################
  handle.Phylogeny_TRANSFORM_kappa <- rescale(handle.Phylogeny, "kappa")(sigsq = numeric.Sig2, kappa = numeric.kappa)
  
  ##############################
  # transform via STACK lrates #
  ##############################
  handle.Phylogeny_TRANSFORM_kappa_lrates <- rescale(handle.Phylogeny_TRANSFORM_kappa, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_kappa_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
