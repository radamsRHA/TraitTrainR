#' Simulate.TraitTrain.kappa_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) kappa model
#'
#' This function returns a Matrix of simulated trait data. Species are rows, traits are columns in matrix.TraitData 
#' @param handle.Phylogeny Phylogeny used to simulate training data under kappa model
#' @param numeric.NumberOfSpecies Numeric counting the number of species
#' @param numeric.Sig2 Numeric Value of the sigsg (evolutionary rate) parmater of the kappa model
#' @param numeric.AncestralState Numerical value of the z0 (ancestral state) parameter of the kappa model
#' @param numeric.kappa Numerical value of the kappa parameter of the kappa model
#' @param vector.STACK_lrate_nodes Vector that includes all nodes for a STACKED lrates model (on top of kappa)
#' @param vector.STACK_lrate_rates Vector that includes all rate values for each node in vector.STACK_lrate_nodes for a STACKED lrates model (on top of kappa)
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STACKED AncShift model (on top of kappa)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STACKED AncShift model (on top of kappa)
...
#' #############################
#' list.SimulationModelSettings[[1]] <- list(string.SimulationModel = "kappa", 
...
#
#' #############################################################################
#' ## simulate under kappa model with STACKED 11 (kappa + lrates + AncShift) #
#' #############################################################################
#' Simulate.TraitTrain.kappa_STACKED_11(numeric.NumberOfSpecies = length(handle.PrimatePhylogeny$tip.label), 
...
#'                                      numeric.kappa = list.SimulationModelSettings[[1]]$vector.kappa[1],
#'                                     matrix.R = list.SimulationModelSettings[[1]]$list.Rmatrix[[1]])

########################################
# Simulate.TraitTrain.kappa_STACKED_11 #
########################################
Simulate.TraitTrain.kappa_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.kappa, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  
  ########################
  # transform via kappa #
  ########################
  handle.Phylogeny_TRANSFORM_kappa <- phytools::rescale(handle.Phylogeny, "kappa")(sigsq = numeric.Sig2, kappa = numeric.kappa)
  
  ##############################
  # transform via STACK lrates #
  ##############################
  handle.Phylogeny_TRANSFORM_kappa_lrates <- phytools::rescale(handle.Phylogeny_TRANSFORM_kappa, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(geiger::sim.char(phy = handle.Phylogeny_TRANSFORM_kappa_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
