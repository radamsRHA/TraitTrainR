#' Simulate.TraitTrain.lambda_STACKED_11: Function to simulate trait data under the STACK (lrates + shifted) lambda model
#'
...
#' @param numeric.AncestralState Numerica value of the z0 (ancestral state) parameter of the lambda model
#' @param numeric.lambda Numerica value of the lambda parameter of the lambda model
...
#' @param vector.STACK_AncShiftNode Vector that includes all nodes for a STAKCED AncShift model (on top of lambda)
#' @param vector.STACK_AncShiftValue Vector that includes all shift values for each node in vector.STACK_AncShiftNode for a STAKCED AncShift model (on top of lambda)
...
#########################################
Simulate.TraitTrain.lambda_STACKED_11 <- function(numeric.NumberOfSpecies, handle.Phylogeny, numeric.Sig2, numeric.lambda, numeric.AncestralState, vector.STACK_lrate_nodes, vector.STACK_lrate_rates, vector.STACK_AncShiftNode, vector.STACK_AncShiftValue, matrix.R){
  
  ########################
  # transform via lambda #
  ########################
  handle.Phylogeny_TRANSFORM_lambda <- rescale(handle.Phylogeny, "lambda")(sigsq = numeric.Sig2, lambda = numeric.lambda)
  
  ##############################
  # transform via STACK lrates #
  ##############################
  handle.Phylogeny_TRANSFORM_lambda_lrates <- rescale(handle.Phylogeny_TRANSFORM_lambda, "lrate")(node = vector.STACK_lrate_nodes, rate = vector.STACK_lrate_rates)
  
  ##################
  # simulate trait #
  ##################
  matrix.TraitData <- as.matrix(sim.char(phy = handle.Phylogeny_TRANSFORM_lambda_lrates, par = matrix.R, nsim = 1, model = "BM", root = numeric.AncestralState)[,,1])
